...
namespace TailwindTraders.Mobile.Features.Product.Detail
{
    public class ProductDetailViewModel : BaseStateAwareViewModel<ProductDetailViewModel.State>
    {
        ...
        private async Task LoadSecondaryDataAsync(ProductDTO product)
        {
            var status = await TryExecuteWithLoadingIndicatorsAsync(RequestSimilarAndAlsoBoughtProductsAsync(product));
            CurrentState = status ? State.EverythingOK : State.Error;
        }

        private async Task RequestProductDetailAsync()
        {
            var product = await RestPoolService.ProductsAPI.GetDetailAsync(productId);
            ...
            await LoadSecondaryDataAsync(product);
        }

        private async Task RequestSimilarAndAlsoBoughtProductsAsync(ProductDTO product)
        {
            ...
            SimilarProducts = (await RestPoolService.ProductsAPI.GetProductsByCategoryAsync(product.ProductType.ToString()))
                .Products.Shuffle().Take(5).Select(item => new ProductViewModel(item, FeatureNotAvailableCommand));

            AlsoBoughtProducts = (await RestPoolService.ProductsAPI.GetProductsAsync())
                .Products.Shuffle().Take(3);
        }
